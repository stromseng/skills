        ...
        return AuthMiddleware.of({
            execute: (request) =>
                Effect.gen(function* () {
                    ...
                    if (!token) {
                        return yield* Effect.fail(new UnauthorizedError({ userMessage: "Missing token" }))
                    }

                    const user = yield* authService.validateToken(token).pipe(
                        Effect.catchTag("TokenExpiredError", () =>
                            Effect.fail(new UnauthorizedError({ userMessage: "Token expired" }))
                        ),
                        Effect.catchTag("TokenInvalidError", () =>
                            Effect.fail(new UnauthorizedError({ userMessage: "Invalid token" }))
                        ),
                    )
