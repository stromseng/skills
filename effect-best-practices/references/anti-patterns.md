...
yield* Effect.gen(function* () {
    const user = yield* repo.findById(id)
    if (!user) {
        return yield* Effect.fail(new UserNotFoundError({ userId: id, userMessage: "Not found" }))
    }
    return user
})
```
...
yield* someEffect.pipe(
    Effect.catchTags({
        DatabaseError: (err) => Effect.fail(new ServiceUnavailableError({ reason: err.reason, cause: err })),
        ValidationError: (err) => Effect.fail(new BadRequestError({ reason: err.reason, cause: err })),
    }),
)
```
...
yield* Option.match(maybeUser, {
    onNone: () => Effect.fail(new UserNotFoundError({ userId, userMessage: "Not found" })),
...
yield* someEffect.pipe(
    Effect.catchTag("RecoverableError", (err) =>
        Effect.fail(new DomainError({ reason: err.reason, cause: err }))
    ),
)
```
...
yield* effect.pipe(
    Effect.catchTag("SpecificError", (err) =>
        Effect.fail(new MappedError({ reason: err.reason, cause: err }))
    ),
)
```
